        ...
        */
        ...
        public int sta_readAttLog(DataTable dt_log)
        {
            ...
            if (axCZKEM1.ReadGeneralLogData(GetMachineNumber()))
            {
                ...
                {
                    axCZKEM1.SSR_GetUserInfo(iMachineNumber, sdwEnrollNumber, out strName, out strPassword, out iPrivilege, out bEnabled);
                    DataRow dr = dt_log.NewRow();
                    dr["ID"] = i++;
                    dr["BIOMETRIC_ID"] = sdwEnrollNumber;
                    dr["EMPLOYEE_NAME"] = strName;
                    dr["WORK_DATE"] = idwMonth + "-" + idwDay + "-" + idwYear;
                    dr["WORK_TIME"] = gm.toDateString(idwHour + ":" + idwMinute + ":" + idwSecond, "hh:mm tt");
                    dr["STATUS"] = (idwInOutMode == 0) ? "IN" : "OUT";
                    dt_log.Rows.Add(dr);
                    /*
                     DataRow dr = dt_log.NewRow();
                    dr["empid"] = sdwEnrollNumber;
                    dr["name"] = idwYear + "-" + idwMonth + "-" + idwDay + " " + idwHour + ":" + idwMinute + ":" + idwSecond;
                    dr["date"] = idwVerifyMode;
                    dr["time"] = idwInOutMode;
                    dr["status"] = idwWorkcode;
                    dt_log.Rows.Add(dr);
                     */
                }
                ret = 1;
            ...
            else
            {
                ...
                if (idwErrorCode != 0)
                {
                    System.Diagnostics.Debug.WriteLine("*Read attlog failed,ErrorCode: " + idwErrorCode.ToString());
                    //lblOutputInfo.Items.Add("*Read attlog failed,ErrorCode: " + idwErrorCode.ToString());
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine("No data from terminal returns!");
                    //lblOutputInfo.Items.Add("No data from terminal returns!");
                }
            }
        ...
        public int sta_readLogByPeriod(DataTable dt_log, string fromTime, string toTime)
        {
            if (GetConnectState() == false)
            {
                //lblOutputInfo.Items.Add("*Please connect first!");
