...
namespace Human_Resource_Information_System
{
    public partial class RPT_RES_entry : Form
    {
        ...
        public RPT_RES_entry(String rnum)
        {
            ...
            fileloc_payroll = system_loc + "\\HRPublish\\Reports\\Payroll\\";
            fileloc_timeclock = system_loc + "\\HRPublish\\Reports\\Timeclock\\";
        ...
        private void bgworker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            //{
                ...
                else if (rpt_no == "P101")
                {
                    ...
                    add_fieldparam("comp_addr", comp_addr);
                    add_fieldparam("userid", GlobalClass.username);
                    add_fieldparam("userfullname", db.get_colval("x08", "opr_name", "uid='" + GlobalClass.username + "'"));
                    disp_reportviewer(myReportDocument);
                   
                    //db.get_colval("m99", "comp_sss", "");
                ...
                {
                    inc_pbar(10);
                    myReportDocument.Load(fileloc_timeclock + "dtr_summary_report.rpt");
        ...
        public void print_employee_listing( String payperiod_desc, String payperiod_code, String depttxt_frm, String deptcode_frm, String depttxt_to, String deptcode_to, String empid)
        {
            String WHERE = "", strSQL = "";
            String payrollperiod ="", dept_name = "";
            ...
            if (!String.IsNullOrEmpty(payperiod_code))
            {
                ...
                payrollperiod = "Payroll Period : " + payperiod_desc;
            }
            if (!String.IsNullOrEmpty(empid))
            {
        ...
        public void print_payslip(String payperiod_code, String depttxt_frm, String deptcode_frm, String depttxt_to, String deptcode_to, String empid, String byRateType, String sortBy, Boolean isEmpOnly, Boolean isDetailed)
        {
            ...
            strSQL = "SELECT ep.*,CASE WHEN ep.notes NOTNULL THEN CONCAT('Note: ', ep.notes) END as payrate_dts ,d.dept_name, to_char(pp.date_from,'YYYY-MM-DD') AS date_from, to_char(pp.date_to,'YYYY-MM-DD') AS date_to, to_char(pp.date_from,'Month DD, YYYY') AS date_from_desc, to_char(pp.date_to,'Month DD, YYYY') AS date_to_desc, e.firstname, e.lastname, e.mi, e.tax_bracket,e.biometric FROM rssys.hr_emp_payroll ep LEFT JOIN rssys.hr_employee e ON e.empid=ep.empid LEFT JOIN rssys.hr_payrollpariod pp ON pp.pay_code=ep.ppid LEFT JOIN rssys.hr_department d ON e.department=d.deptid WHERE 1=1 " + WHERE + " " + ORDERBY + "";
        ...
        public void print_employee_dtr(String branchcode, String branchtxt, String deptcode_frm, String depttxt_frm, String deptcode_to, String depttxt_to, String empid, String empname, String poutcode, String poutdescription, String dtp_from, String dtp_to, Boolean isIncludeNoLogs = true)
        {
            String WHERE = "", WHERE2 = "", ORDERBY = "", strSQL = "";
            String dept_name = "", branchname="All branches", potdescription = "All Payout Type";
            String minutes = "_min";


            WHERE += " AND e.gen_date BETWEEN '" + dtp_from + "' AND '" + dtp_to + "' ";

            if (!String.IsNullOrEmpty(branchcode))
            {
                WHERE += " AND e.branch = '" + branchcode + "'  ";
            ...
            if (!String.IsNullOrEmpty(poutcode))
            {
                WHERE += " AND e.potcode = '" + poutcode + "'  ";
                potdescription = poutdescription + "";
            }
            ...
            strSQL = db.get_print_view_emp_dtr_summary_str(WHERE, true);

            /*strSQL = @"SELECT to_char(e.gen_date,'MM/DD/YY DY') AS gen_date, dayoff, 
		                ...
		                work_hr - late - undertime + overtime AS work_nettotal, absent, leavecredit,
		                leave_desc, leave_type, mon
		                FROM rssys.view_emp_dtr_summary e 
                        WHERE e.gen_date BETWEEN '" + dtp_from + "' AND '" + dtp_to + "' " + WHERE + @" 
            ...
            /*
            strSQL = @"SELECT *, work_hr_min-late_min-undertime_min+overtime_min AS work_nettotal_min, work_hr-late-undertime+overtime AS work_nettotal FROM (
                    SELECT to_char(pp.gen_date,'MM/DD/YY DY') AS gen_date,
                        CASE WHEN dy.day=dayoff1 OR dy.day=dayoff2 THEN 'Day Off' ELSE '' END AS dayoff, 
                        e.lastname||','||e.firstname||' '||e.mi  ||' - '|| e.status_name AS empid,  e.dept_name ||' - '||  e.position_name ||' ['|| e.empid ||']' AS department, 
                        ...
                        CASE WHEN e.fixed_sched='Y'  AND (dy.day!=dayoff1 AND dy.day!=dayoff2) THEN
				            8
			            ELSE
				            CASE WHEN wk.work_hr IS NULL THEN '0' 
					            ELSE round((extract(epoch from wk.work_hr::time)/3600)::numeric,4) END 
			            END * 60 AS work_hr_min,
                        ...
                        CASE WHEN e.fixed_sched='Y' THEN
				            CASE WHEN trim(to_char(pp.gen_date,'day'))='saturday' THEN 
						            CASE 
							            WHEN wk.time_in::time-e.shift_sched_sat_from::time>'00:00'::time 
								            THEN round((extract(epoch from to_char(wk.time_in::time-e.shift_sched_sat_from::time,'hh24:mi')::time)/3600)::numeric,4)  
							            ELSE '0' END        
						            ELSE 
							            CASE WHEN wk.time_in::time-e.shift_sched_from::time>'00:00'::time 
								            THEN round((extract(epoch from to_char(wk.time_in::time-e.shift_sched_from::time,'hh24:mi')::time)/3600)::numeric,4)  
							            ELSE '0' END 
						            END 
			            ELSE 			
				            CASE WHEN trim(to_char(pp.gen_date,'day'))='saturday' THEN 
						            CASE 
							            WHEN wk.time_in::time-e.sched_timein_sat::time>'00:00'::time 
								            THEN round((extract(epoch from to_char(wk.time_in::time-e.sched_timein_sat::time,'hh24:mi')::time)/3600)::numeric,4)  
							            ELSE '0' END        
						            ELSE 
							            CASE WHEN wk.time_in::time-e.sched_timein::time>'00:00'::time 
								            THEN round((extract(epoch from to_char(wk.time_in::time-e.sched_timein::time,'hh24:mi')::time)/3600)::numeric,4)  
							            ELSE '0' END 
						            END 
			            END * 60 AS late_min, 
			            ...
			            CASE WHEN e.fixed_sched='Y' THEN
					            CASE WHEN trim(to_char(pp.gen_date,'day'))='saturday' THEN 
							            CASE 
								            WHEN wk.time_out::time-e.shift_sched_sat_to::time<'00:00'::time 
									            THEN 
										            round((extract(epoch from to_char(e.shift_sched_sat_to::time-wk.time_out::time,'hh24:mi')::time)/3600)::numeric,4)
								            ELSE '0' END      
							            ELSE 
								            CASE WHEN wk.time_out::time-e.shift_sched_to::time<'00:00'::time 
									            THEN round((extract(epoch from to_char(e.shift_sched_to::time-wk.time_out::time,'hh24:mi')::time)/3600)::numeric,4)  
								            ELSE '0' END 
							            END 
			            ELSE
					            CASE WHEN trim(to_char(pp.gen_date,'day'))='saturday' THEN 
							            CASE 
								            WHEN wk.time_out::time-e.sched_timeout_sat::time<'00:00'::time 
									            THEN 
										            round((extract(epoch from to_char(e.sched_timeout_sat::time-wk.time_out::time,'hh24:mi')::time)/3600)::numeric,4)
								            ELSE '0' END      
							            ELSE 
								            CASE WHEN wk.time_out::time-e.sched_timeout::time<'00:00'::time 
									            THEN round((extract(epoch from to_char(e.sched_timeout::time-wk.time_out::time,'hh24:mi')::time)/3600)::numeric,4)  
								            ELSE '0' END 
							            END 
			            END * 60 AS undertime_min, 
					            ...
					            CASE WHEN trim(to_char(pp.gen_date,'day'))='saturday' THEN 
							            CASE 
								            WHEN e.sched_timeout_sat::time-wk.time_out::time>'00:00'::time 
									            THEN round((extract(epoch from to_char(e.sched_timeout_sat::time-wk.time_out::time,'hh24:mi')::time)/3600)::numeric,4)  
								            ELSE '0' END       
							            ELSE 
                        ...
                        CASE WHEN e.fixed_sched='Y' THEN                              
                            CASE WHEN dy.day=dayoff1 OR dy.day=dayoff2 THEN 'Day Off' 
                            ELSE 
                                CASE WHEN trim(to_char(pp.gen_date,'day'))='saturday' THEN 
                                    TO_CHAR(e.shift_sched_sat_from::TIME, 'hh12:mi AM') ||' to '|| TO_CHAR(e.shift_sched_sat_to::TIME, 'hh12:mi AM') 
						        ELSE 
                                    TO_CHAR(e.shift_sched_from::TIME, 'hh12:mi AM') ||' to '|| TO_CHAR(e.shift_sched_to::TIME, 'hh12:mi AM')
						        END                                                   
                            END 
                        ELSE
                                CASE WHEN dy.day=dayoff1 OR dy.day=dayoff2 THEN 'Day Off' 
                                ELSE TO_CHAR(sched.time_in::TIME, 'hh12:mi AM') ||' to '|| TO_CHAR(sched.time_out::TIME, 'hh12:mi AM') END 
                        END AS mon 

                        FROM (SELECT generate_series('" + dtp_from + "'::date, '" + dtp_to + @"'::date, '1 day'::interval)::date AS gen_date ) pp 
                        JOIN rssys.view_emp_profile_payroll  e ON (e.date_hired<>'1900-01-01' AND e.date_hired<=pp.gen_date) 	AND ((e.empstatus NOT LIKE '9%'))
                        ...
                        LEFT JOIN (
                                    SELECT lv.*, lt.description FROM rssys.hr_leaves lv LEFT JOIN rssys.hr_leave_type lt ON (lt.code=lv.leave_type)WHERE 1=1 " + WHERE2 + @"
                                ) lv ON (e.empid=lv.empid AND pp.gen_date BETWEEN lv.leave_from AND lv.leave_to)  
                        LEFT JOIN rssys.hr_emp_shift es ON es.empid=e.empid
                        LEFT JOIN rssys.hr_shift_schedule sched ON sched.code=es.shiftcode
                        WHERE 1=1  AND e.fixed_sched!='E' " + WHERE + @"  
                        ORDER BY e.dept_name, e.position_name, e.lastname, pp.gen_date
             ) tbl"; */

            try {   rtxt_msg.Text = strSQL;    } catch { }            

            data.strSQL = strSQL;
            data.dept_name = branchtxt + ": " + dept_name +" for " +  poutdescription;
            data.date = gm.toDateString(dtp_from, "MMMM dd, yyyy") + " to " + gm.toDateString(dtp_to, "MMMM dd, yyyy");
            ...
            strSQL = @"SELECT * FROM
                      (
                       SELECT to_char(pp.gen_date,'MM/DD/YY DY') AS gen_date,
                        ...
                        LEFT JOIN rssys.view_emp_dtr wk ON wk.work_date=pp.gen_date AND wk.empid=e.empid
                        LEFT JOIN rssys.hr_days dy ON (dy.dayname=TRIM(to_char(pp.gen_date,'DAY'))) 
                        LEFT JOIN (SELECT lv.*, lt.description FROM rssys.hr_leaves lv LEFT JOIN rssys.hr_leave_type lt ON (lt.code=lv.leave_type) WHERE 1=1 " + WHERE2 + @") lv 
                        ON (e.empid=lv.empid AND pp.gen_date BETWEEN lv.leave_from AND lv.leave_to)  
                         WHERE 1=1  " + WHERE + "    ORDER BY e.dept_name, e.position_name, e.lastname, pp.gen_date ) tbl WHERE tbl.absent > 0 OR tbl.late > 0 OR tbl.undertime > 0 ";
            /*
        ...
        public void print_daily_timelog(String deptcode_frm, String depttxt_frm, String deptcode_to, String depttxt_to, String empid, String empname, String dtp_from, String dtp_to)
        {
            ...
            strSQL = @"SELECT department, empid, empname, position_name, SUM(work_hr) AS work_hr, SUM(absent) AS absent,  SUM(late) AS late, SUM(undertime) AS undertime, SUM(overtime) AS overtime   FROM
              (
                    SELECT to_char(pp.gen_date,'MM/DD/YY DY') AS gen_date,
                        CASE WHEN dy.day=dayoff1 OR dy.day=dayoff2 THEN 'Day Off' ELSE '' END AS dayoff, 
                        e.empid, e.lastname||','||e.firstname||' '||e.mi  AS empname,  
                        e.dept_name ||' - '|| e.status_name AS department, e.position_name,
                        ...
                        LEFT JOIN (SELECT lv.*, lt.description FROM rssys.hr_leaves lv LEFT JOIN rssys.hr_leave_type lt ON (lt.code=lv.leave_type) WHERE 1=1 " + WHERE2 + @") lv 
                        ON (e.empid=lv.empid AND pp.gen_date BETWEEN lv.leave_from AND lv.leave_to)  
                         WHERE 1=1  " + WHERE + "    ORDER BY e.dept_name, e.position_name, e.lastname, pp.gen_date) tbl GROUP BY department, empid, empname, position_name";
