...
namespace Human_Resource_Information_System
{
    public partial class RPT_RES_entry2 : Form
    {
        ...
        public RPT_RES_entry2(String _rpt_code)
        {
            ...
            {
                this.Text = "Payroll Employee Listing";

                lbl_1.Text = "Payroll Period";
                lbl_2.Text = "Specific Department";
                lbl_3.Text = "Specific Employee";
                lbl_1.Show(); lbl_2.Show(); lbl_3.Show(); 
            ...
            else if (rpt_code == "T101")
            {
                ...
                lbl_3.Text = "Work Dates";
                lbl_4.Text = "Specific Employee";
                lbl_5.Text = "Pay Out Type";

                lbl_1.Show(); lbl_2.Show(); lbl_3.Show(); lbl_3to.Show(); lbl_4.Show(); lbl_5.Show();

                gc.load_branch(cbo_1);
                load_department(cbo_2frm);
                load_department(cbo_2to);
                load_employee(cbo_4l);
                gc.load_payroll_type(cbo_5l); 
                cbo_1.Show(); cbo_2frm.Show(); cbo_2to.Show();  cbo_4l.Show(); cbo_5l.Show();

                chk_1.Text = "Include the No Log Employees";
                chk_1.Show();
                chk_1.Checked = true ;
        ...
        private void btn_proceed_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure you want to proceed?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                {
                    RPT_RES_entry frm = new RPT_RES_entry(rpt_code, this.Text);
                    frm.print_employee_listing(cbo_1.Text, (cbo_1.SelectedValue ?? "").ToString(), cbo_2frm.Text, (cbo_2frm.SelectedValue ?? "").ToString(), cbo_2to.Text, (cbo_2to.SelectedValue ?? "").ToString(),(cbo_3.SelectedValue ?? "").ToString());
                    frm.ShowDialog();
                }
                ...
                {
                    RPT_RES_entry frm = new RPT_RES_entry(rpt_code, this.Text);
                    frm.print_employee_dtr((cbo_1.SelectedValue ?? "").ToString(), cbo_1.Text, (cbo_2frm.SelectedValue ?? "").ToString(), cbo_2frm.Text,  (cbo_2to.SelectedValue ?? "").ToString(), cbo_2to.Text, (cbo_4l.SelectedValue ?? "").ToString(), cbo_4l.Text, (cbo_5l.SelectedValue ?? "").ToString(), cbo_5l.Text, dtp_3frm.Value.ToString("yyyy-MM-dd"), dtp_3to.Value.ToString("yyyy-MM-dd"),chk_1.Checked);
        ...
        public void load_payroll_period(ComboBox cbo)
        {
            try
            {
                cbo.DataSource = db.QueryBySQLCode("SELECT pay_code,to_char(date_from,'YYYY/MM/DD')||' - '||to_char(date_to,'YYYY/MM/DD') AS pay_period FROM rssys.hr_payrollpariod WHERE COALESCE(cancel,cancel,'') <>'Y' ORDER BY date_to");
