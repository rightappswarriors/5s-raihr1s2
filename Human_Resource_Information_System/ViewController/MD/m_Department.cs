...
namespace Human_Resource_Information_System
{
    public partial class m_Department : Form
    {
        ...
        private void m_Department_Load(object sender, EventArgs e)
        {
            ...
            cbo_searchby.Items.Insert(1,"NAME");
            gc.load_account_title(cbo_acclink);
            disp_list();
        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (cbo_acclink.SelectedIndex == -1)
            {
                MessageBox.Show("Please enter the required fields.");
                return;
            }
            ...
            code = txt_code.Text;
            name = txt_name.Text;
            atcode = cbo_acclink.SelectedValue.ToString();
            col = "deptid,dept_name,at_code";
            val = "" + db.str_E(code) + "," + db.str_E(name) + "," + db.str_E(atcode) + "";
            ...
            {
                col = "deptid=" + db.str_E(code) + ", dept_name=" + db.str_E(name) + ",at_code = "+db.str_E(atcode)+"";
            ...
            try
            {
                ...
                if (dgv_list.Rows.Count > 1 && !String.IsNullOrEmpty(id))
                {
                    ...
                    try
                    {
                        ...
                        cbo_acclink.SelectedValue = db.get_colval("hr_department","at_code","deptid = '"+id+"'");
                    }
                    catch (Exception er) { }
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            if (cbo_searchby.SelectedIndex == 0)
            {
                ...
                col = "deptid LIKE '%" + search + "%'";
                disp_search_list(col);
            }
            if (cbo_searchby.SelectedIndex == 1)
            {
                col = "dept_name LIKE '%" + search + "%'";
                disp_search_list(col);
            }
            //WHERE CustomerName LIKE '%or%  deptid character varying(8) NOT NULL,dept_name
        ...
        private void disp_search_list(String col)
        {
            ...
            DataTable dt = db.QueryBySQLCode("SELECT * FROM rssys.hr_department WHERE COALESCE(cancel,cancel,'')<>'Y' and "+col);
