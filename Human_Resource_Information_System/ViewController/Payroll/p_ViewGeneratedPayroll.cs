...
namespace Human_Resource_Information_System
{
    public partial class p_ViewGeneratedPayroll : Form
    {
        ...
        private Double daily_rate = 0.00, pay_rate = 0.00, hour_rate = 0.00, minute_rate = 0.00, gross_pay = 0.00, basic_pay = 0.00, monthly_rate = 0.00, late_amt = 0.00, absent_amt = 0.00, lat_ut = 0.00, reg_ot_a = 0.00, reg_ot_b_total = 0.00,regpay=0.00;
            ...
            try
            {
                ...
                if(payroll.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        //e
                        regpay = Convert.ToDouble(payroll.Rows[0]["hour_rate"].ToString());
                        ...
                        txt_remarks.Text = payroll.Rows[0]["notes"].ToString();
                        
                        txt_pay_rate.Text = (payroll.Rows[0]["pay_rate"]??"0.00").ToString();

                        txt_rate_type.Text = payroll.Rows[0]["description"].ToString();
                        ...
                        //minute_rate.ToString("N", new CultureInfo("en-US"));
                    ...
                    if (payroll.Rows[0]["ratecode"].ToString() == "D")
                    {
                        daily_rate = pay_rate;                    
                    }
                    else if (payroll.Rows[0]["ratecode"].ToString() == "M")
                    {
                    ...
                    //payroll.Rows[0]["regular_pay"].ToString();
                    // reg_ot_a = Convert.ToDouble(txt_reg_ot_a.Text);
                    reg_ot_b_total = hour_rate * Convert.ToDouble(txt_reg_ot_a.Text.ToString()) * 1.25;
                    if (payroll.Rows[0]["ratecode"].ToString() == "M")
                    {
                        if (payroll.Rows[0]["fixed_rate"].ToString() == "1")
                        {
                            txt_regpay.Text = regpay.ToString("0.00");
                            ...
                            basic_pay = (regpay - (late_amt + absent_amt));
                        ...
                        else
                        {
                            try
                            {
                                //error in days_worked
                                Double d = Convert.ToDouble(txt_dayswoked.Text.ToString());
                                days_worked = (int)d;
                    ...
                    if (payroll.Rows[0]["leave_type"].ToString() != "none" && payroll.Rows[0]["leave_type"].ToString() != "")
                    {
                        groupBox7.Visible = true;
                        lbl_leave_type.Text = get_leave_type(payroll.Rows[0]["leave_type"].ToString());
                        txt_leave_days.Text = payroll.Rows[0]["leave_days"].ToString();
                        txt_leave_amt.Text = payroll.Rows[0]["leave_amnt"].ToString();
                    }
                    else {
                        groupBox7.Visible = false;
                    }

                    if (checkif13month(ppid))
                    {
                        label3.Visible = true;
                        txt_13_month.Visible = true;
                        txt_13_month.Text = payroll.Rows[0]["amnt13month"].ToString(); 

                    }
                    else {
                        label3.Visible = false;
                        txt_13_month.Visible = false;
                    }
                    ///daily_rate
                    Double sprate = daily_rate * (30 / 100.00);
                    Double hrate = Math.Round(hour_rate * Convert.ToDouble((payroll.Rows[0]["legal_hol_ot_a"] ?? "0.00").ToString()),2);
                    Double spay = Math.Round(hour_rate * Convert.ToDouble((payroll.Rows[0]["special_hol_ot_a"] ?? "0.00").ToString()),2);
                    Double shot_b = Math.Round(hour_rate * Convert.ToDouble((payroll.Rows[0]["special_hol_ot_a"] ?? "0.00").ToString()),2);
                    ...
                    /// txt_legalhol_ot_a = txt_legalhol_pay_a
        ...
        private Boolean checkif13month(String ppid){
            DataTable payroll = db.QueryBySQLCode("SELECT gen_13_month FROM rssys.hr_payrollpariod where pay_code = '"+ppid+"'");
            if (payroll.Rows.Count > 0) {
        ...
        private String get_leave_type(String code) {
            DataTable leave = db.QueryBySQLCode("SELECT * FROM rssys.hr_leave_type where code = '"+code+"'");
            return leave.Rows[0]["description"].ToString();
        }
        ...
        private void btn_mainsave_Click(object sender, EventArgs e)
        {
            ...
            late_amnt = txt_late_ut_amt.Text != "" ? txt_late_ut_amt.Text : "0.00";
        ...
        {
            Double gross = gm.toNormalDoubleFormat(txt_basic_pay.Text) + gm.toNormalDoubleFormat(txt_reg_ot_b.Text) + gm.toNormalDoubleFormat(txt_dayoffot_b.Text) + gm.toNormalDoubleFormat(txt_legalhol_ot_b.Text) + gm.toNormalDoubleFormat(txt_specialhol_ot_b.Text) + gm.toNormalDoubleFormat(txt_night_diff_b.Text) + gm.toNormalDoubleFormat(txt_other_earning.Text) + gm.toNormalDoubleFormat(txt_vl_b.Text) + gm.toNormalDoubleFormat(txt_sl_b.Text) + gm.toNormalDoubleFormat(txt_pl_b.Text) + gm.toNormalDoubleFormat(txt_13_month.Text)+gm.toNormalDoubleFormat(txt_leave_amt.Text) + gm.toNormalDoubleFormat(txt_legalhol_pay_b.Text) + gm.toNormalDoubleFormat(txt_specialhol_pay_b.Text);
        ...
        /*
          void calculate_gross()
        {
            Double gross = gm.toNormalDoubleFormat(txt_basic_pay.Text) + gm.toNormalDoubleFormat(txt_reg_ot_b.Text) + gm.toNormalDoubleFormat(txt_dayoffot_b.Text) + gm.toNormalDoubleFormat(txt_legalhol_ot_b.Text) + gm.toNormalDoubleFormat(txt_specialhol_ot_b.Text) + gm.toNormalDoubleFormat(txt_legalhol_pay_b.Text) + gm.toNormalDoubleFormat(txt_specialhol_pay_b.Text) + gm.toNormalDoubleFormat(txt_night_diff_b.Text) + gm.toNormalDoubleFormat(txt_other_earning.Text) + gm.toNormalDoubleFormat(txt_vl_b.Text) + gm.toNormalDoubleFormat(txt_sl_b.Text) + gm.toNormalDoubleFormat(txt_pl_b.Text) + gm.toNormalDoubleFormat(txt_other_earning.Text);
        ...
        private void calculate_basic_pay() {
            Double regpay = gm.toNormalDoubleFormat(txt_regpay.Text);
            Double absamt = gm.toNormalDoubleFormat(txt_absent_amount.Text);
            Double lateamt = gm.toNormalDoubleFormat(txt_late_ut_amt.Text);
            Double total = (regpay - (lateamt + absamt));

            total = regpay * gm.toNormalDoubleFormat(txt_dayswoked.Text);
            txt_basic_pay.Text = total.ToString();
        }
        private void txt_absent_TextChanged(object sender, EventArgs e)
        {
            Double daily_rate = Convert.ToDouble(txt_daily_rate.Text);
            Double absences = Convert.ToDouble(txt_absent.Text);
            Double total = daily_rate * absences;
            txt_absent_amount.Text = total.ToString();
            calculate_basic_pay();
        ...
        private void txt_daily_rate_Click(object sender, EventArgs e)
        {
        ...
        private void txt_13_month_TextChanged(object sender, EventArgs e)
        {
            calculate_gross();
            calculate_net();
        }
        ...
        {
            Double hour_rate = Convert.ToDouble(txt_hourly_rate.Text);
            Double late_ut = Convert.ToDouble(txt_late_ut.Text);
            Double total = hour_rate * late_ut;
            txt_late_ut_amt.Text = total.ToString();
            calculate_basic_pay();
        }

        private void txt_regpay_TextChanged(object sender, EventArgs e)
        {
            calculate_basic_pay();
        }

        private void txt_absent_amount_TextChanged(object sender, EventArgs e)
        {
            calculate_basic_pay();
        }

        private void txt_late_ut_amt_TextChanged(object sender, EventArgs e)
        {
            calculate_basic_pay();
        }
