...
namespace Human_Resource_Information_System
{
    public partial class rpt_payroll_summary : Form
    {
        ...
        private void bgworker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            document.Add(new Paragraph("AMADEUS & SONS FOOD TRADING INC.", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 11f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_LEFT });
            //document.Add(new Paragraph("AMADEUS & SONS", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 11f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_LEFT });
                ...
                if (cbo_employee.SelectedIndex != -1)
                {
                    foreach (DataRow _emp in employees.Rows)
                    {
                        ...
                        if(emp_payroll.Rows[0]["basic_pay"].ToString() != ""){
                            basic_pay = Convert.ToDouble(emp_payroll.Rows[0]["basic_pay"].ToString());
                            gross = gross + basic_pay;
                        }
                        if (emp_payroll.Rows[0]["netpay"].ToString() != "")
                        {
                            netpay = Convert.ToDouble(emp_payroll.Rows[0]["netpay"].ToString());
                        }
                        if (emp_payroll.Rows[0]["w_tax"].ToString() != "")
                        {
                            wtax = Convert.ToDouble(emp_payroll.Rows[0]["w_tax"].ToString());
                        }
                        if (emp_payroll.Rows[0]["sss_cont_a"].ToString() != "")
                        {
                            sss = Convert.ToDouble(emp_payroll.Rows[0]["sss_cont_a"].ToString());
                        }
                        if (emp_payroll.Rows[0]["sss_cont_a"].ToString() != "")
                        {
                            phil = Convert.ToDouble(emp_payroll.Rows[0]["philhealth_cont_a"].ToString());
                        }
                        if (emp_payroll.Rows[0]["pag_ibig_a"].ToString() != "")
                        {
                            hdmf = Convert.ToDouble(emp_payroll.Rows[0]["pag_ibig_a"].ToString());
                        }
                        if (emp_payroll.Rows[0]["other_deduction"].ToString() != "")
                        {
                            deduc = Convert.ToDouble(emp_payroll.Rows[0]["other_deduction"].ToString());
                        }
                        if (emp_payroll.Rows[0]["other_earnings"].ToString() != "")
                        {
                            earn = Convert.ToDouble(emp_payroll.Rows[0]["other_earnings"].ToString());
                            gross = gross + earn;
                        }
                        Double late_ut = 0.00;
                        if (emp_payroll.Rows[0]["late_amnt"].ToString() != "")
                        {
                            late_ut = Convert.ToDouble(emp_payroll.Rows[0]["late_amnt"].ToString());
                        }
                        ...
                        _thead.AddCell(new PdfPCell(new Paragraph("0.00", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(gross.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(earn.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(deduc.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(sss.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(phil.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(hdmf.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(wtax.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
                        _thead.AddCell(new PdfPCell(new Paragraph(netpay.ToString("0.00"), new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10f, iTextSharp.text.Font.BOLD)) { Alignment = Element.ALIGN_CENTER }) { HorizontalAlignment = Element.ALIGN_CENTER });
