...
namespace Human_Resource_Information_System
{
    public partial class rpt_leaves_summary : Form
    {
        ...
        private void bgworker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            PdfPTable dis_earnings = new PdfPTable(5);
            float[] _w2 = new float[] { 70, 50, 50, 50, 50};
            dis_earnings.SetWidths(_w2);
            dis_earnings.AddCell(new PdfPCell(new Paragraph("Employee Name")) { Border = 2 });
            dis_earnings.AddCell(new PdfPCell(new Paragraph("Leave Type")) { Border = 2 });
            dis_earnings.AddCell(new PdfPCell(new Paragraph("No. of Days")) { Border = 2 });
            dis_earnings.AddCell(new PdfPCell(new Paragraph("Leave With Pay")) { Border = 2 });
            dis_earnings.AddCell(new PdfPCell(new Paragraph("Leave Amount")) { Border = 2 });
            ...
            foreach (DataRow _employees in employees.Rows)
            {
                ...
                try
                {
                    ...
                    {
                        DataTable has_leave = db.QueryBySQLCode("SELECT DISTINCT(empid),leave_pay,no_of_days,leave_amount FROM rssys.hr_leaves WHERE empid = '" + empno + "' AND leave_type = '" + _leave_type["code"].ToString() + "'");
                        if (has_leave != null && has_leave.Rows.Count > 0)
                        {
                            foreach(DataRow _leave in has_leave.Rows){
                                dis_earnings.AddCell(new PdfPCell(new Paragraph(fname + " " + lname)) { Border = 2 });
                                ...
                                {
                                    dis_earnings.AddCell(new PdfPCell(new Paragraph(_hoe["description"].ToString())) { Border = 2 });
                                    /*
                                ...
                                dis_earnings.AddCell(new PdfPCell(new Paragraph(_leave["no_of_days"].ToString())) { Border = 2 });
                                dis_earnings.AddCell(new PdfPCell(new Paragraph(_leave["leave_pay"].ToString())) { Border = 2 });
                                dis_earnings.AddCell(new PdfPCell(new Paragraph(_leave["leave_amount"].ToString())) { Border = 2 });
