    ...
    public partial class t_emp_dtr : Form
    {
        ...
        private void disp_list(Boolean withMsg)
        {
            ...
            for (int r = 0; r < dt.Rows.Count; r++)
            {
                ...
                if (checkdate.Equals(dt.Rows[r]["work_date"].ToString()))
                {
                    continue;
                }
                else
                {
                    ...
                    row.Cells["timein"].Value = gm.toDateString(dt.Rows[r]["timein"].ToString(), "hh:mm tt");
                    row.Cells["timeout"].Value = gm.toDateString(dt.Rows[r]["timeout"].ToString(), "hh:mm tt");
                    ...
                    {
                        row.Cells["source"].Value = "Biometric";
                    }
                }

                checkdate = dt.Rows[r]["work_date"].ToString();
